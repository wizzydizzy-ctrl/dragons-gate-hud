...
local function metrics(width,height,layout)
  ...
  layout.identity_height=layout.heading_font+layout.body_font*4+56
...
function Layout.compute(width,height)
  ...
  if width>=1400 then return metrics(width,height,{mode="wide",left=clamp(width*.17,240,350),right=clamp(width*.15,230,330),top=74,bottom=38,show_character_rail=true,show_room_compass=height>=700,vitals_side="left"}) end
  if width>=1000 then return metrics(width,height,{mode="medium",left=clamp(width*.19,190,250),right=clamp(width*.169,170,220),top=66,bottom=36,show_character_rail=true,show_room_compass=height>=650,vitals_side="left"}) end
