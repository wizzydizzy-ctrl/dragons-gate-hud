...
function Main:start()
  ...
  self.started=true; local ok,err=pcall(function() self:refresh() end); if not ok then self:shutdown(); return nil,err end; self:startChat(); return true
end
function Main:shutdown()
  if self.roundtime_timer then self.adapter:cancelTimer(self.roundtime_timer); self.roundtime_timer=nil end
  if self.chat then self.chat:shutdown(); self.chat=nil end
...
function Main:healthCheck() if not self.started or not self.view or not self.collector or not self.collector.started or not self.chat or not self.chat.started or #self.runtime.events~=(#Events.gmcp+1) then return nil,"HUD is not healthy" end; local ok=pcall(function() self:refresh() end); if not ok then return nil,"state refresh failed" end; return true end
return Main
