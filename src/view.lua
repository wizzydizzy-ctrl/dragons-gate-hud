...
function View:update(s)
  ...
  self.left:echo(View.withFont("<center><span style='color:"..t.accent..";font-size:"..layout.heading_font.."px'><b>"..esc(s.character.full_name).."</b></span><br><span style='color:"..t.jade.."'><b>"..esc(s.character.race).." · "..esc(s.character.class).."</b></span><br><span style='color:"..t.muted.."'>"..esc(s.character.alignment).."</span></center><br><br><span style='color:"..t.accent.."'><b>READINESS</b></span><br><br>Weapon &nbsp; "..ready(v.weapon_readied).."<br><br>Shield &nbsp;&nbsp; "..ready(v.shield_readied).."<br><br><hr><br><span style='color:"..t.accent.."'><b>WEALTH</b></span><br><br>Gold &nbsp; <b>"..v.gold.."</b><br>Silver &nbsp; <b>"..v.silver.."</b><br><br><span style='color:"..t.accent.."'><b>POSITION</b></span><br><br>"..v.position,layout.body_font))
  ...
  if layout.mode=="wide" then self.readiness:echo(View.withFont("<span style='color:"..t.accent.."'><b>COMBAT STATE</b></span><br><br>Roundtime &nbsp; <b>"..(v.roundtime==0 and "READY" or v.roundtime).."</b><br>Position &nbsp; <b>"..v.position.."</b>",layout.body_font))
  else self.readiness:echo(View.withFont("<span style='color:"..t.accent.."'><b>READINESS</b></span><br><br>Weapon &nbsp; "..ready(v.weapon_readied).."<br>Shield &nbsp;&nbsp; "..ready(v.shield_readied).."<br>Roundtime &nbsp; <b>"..(v.roundtime==0 and "READY" or v.roundtime).."</b>",layout.body_font)) end
