...
test("prompt safely completes an unrecognized startup response without waiting for timeout",function()
  local f=fake(); local c=Collector.new(f,Parser,function() end); c:start(); f:line("Welcome to Dragon's Gate, Test!"); f:lines({"unexpected inventory format",">"}); eq(f.sent[2],"stat")
end)
