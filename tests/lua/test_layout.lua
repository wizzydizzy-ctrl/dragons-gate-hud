...
test("wide screens reserve full command deck rails",function()
  local r=Layout.compute(1920,1080); eq(r.mode,"wide"); eq(r.left,326); eq(r.right,250); eq(r.top,74); eq(r.show_character_rail,true); eq(r.vitals_side,"left")
end)
test("medium screens preserve console by folding character rail",function()
  local r=Layout.compute(1200,800); eq(r.mode,"medium"); eq(r.left,228); eq(r.right,180); eq(r.top,66); eq(r.show_character_rail,true); eq(r.show_room_compass,true); eq(r.vitals_side,"left")
end)
