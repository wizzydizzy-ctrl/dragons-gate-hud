...
test("cleanup shutdown removes all seven owned map aliases",function()
  local f=fake(); local hud=Main.new(f,{layout={}}); assert(hud:start()); local before=f:count(f.aliases); eq(before,#Events.aliases+10)
...
test("refresh exposes existing standing and sitting trigger variables to combat",function()
  ...
  function f:getPostureVariables() return {standing=false,sitting=true} end; hud:refresh(); eq(hud.last_state.vitals.standing,false); eq(hud.last_state.vitals.sitting,true); hud:shutdown()
end)
