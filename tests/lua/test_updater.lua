...
local updateSettings={version="0.2.67",github={owner="wizzydizzy-ctrl",repository="dragons-gate-hud"},update={package_limit=1000}}
...
test("startup check skips installation when current and continues startup",function()
  ...
  local adapter={checkLatestAsync=function(_,updater,done) order[#order+1]="check"; done(releaseManifest("0.2.67")) end}
...
test("startup check updates before allowing startup commands",function()
  local order={}; local completed
  local adapter={
    checkLatestAsync=function(_,updater,done) order[#order+1]="check"; done(releaseManifest("0.2.68")) end,
...
test("startup check rejects overlap and completes only once",function()
  ...
  manifestDone(releaseManifest("0.2.67")); manifestDone(releaseManifest("0.2.67")); eq(completions,1)
end)
