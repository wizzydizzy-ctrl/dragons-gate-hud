...
local updateSettings={version="0.2.77",github={owner="wizzydizzy-ctrl",repository="dragons-gate-hud"},update={package_limit=1000}}
...
test("manifest compatibility blocks replacement on an older known Mudlet",function()
  ...
  local ok,err=u:validateManifest(releaseManifest("0.2.78")); eq(ok,nil); assert(err:find("older than required",1,true))
end)
...
local function replacementHarness(options,body)
  ...
  local target=releaseManifest("0.2.78"); target.sha256=SHA.hex("new-package")
  local rollback=releaseManifest("0.2.77"); rollback.sha256=SHA.hex("old-package")
...
test("first updater-managed update bootstraps exact rollback before uninstall",function()
  replacementHarness({},function(h)
    ...
    assert(h.downloads[3].url:find("/releases/download/v0.2.77/manifest.json",1,true)); h.handlers.sysDownloadDone(nil,"/unowned/path"); h:error("https://unrelated.example/failure","ignore me"); eq(h.uninstalls,0)
...
test("startup check skips installation when current and continues startup",function()
  ...
  local adapter={checkLatestAsync=function(_,updater,done) order[#order+1]="check"; done(releaseManifest("0.2.77")) end}
...
test("startup check updates before allowing startup commands",function()
  local order={}; local completed
  local adapter={
    checkLatestAsync=function(_,updater,done) order[#order+1]="check"; done(releaseManifest("0.2.78")) end,
...
test("startup check rejects overlap and completes only once",function()
  ...
  manifestDone(releaseManifest("0.2.77")); manifestDone(releaseManifest("0.2.77")); eq(completions,1)
end)
